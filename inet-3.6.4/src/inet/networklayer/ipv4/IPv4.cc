...
#include "inet/networklayer/ipv4/RTTMessage_m.h"
...
void IPv4::handleMessage(cMessage *msg)
{
    if (dynamic_cast<RegisterTransportProtocolCommand *>(msg)) {
        ...
        endService(PK(msg));
    else if (msg->getArrivalGate()->isName("queueIn") && dynamic_cast<UDPReliableAppPacketAck*>(msg)) { // Host only
        int protocol = dynamic_cast<UDPReliableAppPacketAck*>(msg)->getProtocolId();
...
void IPv4::routeUnicastPacket(IPv4Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv4Address requestedNextHopAddress)
{
    ...
    if (destIE) {
        ...
        // Do ECMP routing
        /*vector<IPv4Route*> routes;
        rt->findBestMatchingEqualCostRoutes(destAddr, routes);
        if (!routes.empty()) {
            uint32 hash = getECMPHash(datagram);
            const IPv4Route* re = routes.at(hash % routes.size());
...
uint32 IPv4::getECMPHash(const IPv4Datagram* datagram)
{
//    uint32 hash = datagram->getSrcAddress().getInt() + datagram->getDestAddress().getInt();
//    const UDPPacket* udpPkt = dynamic_cast<const UDPPacket*>(datagram->getEncapsulatedPacket()); // TODO: Make it also work with TCP
//    if (udpPkt) {
//        hash += udpPkt->getSrcPort() + udpPkt->getDestPort();
//    }
//    return hash;

    // TIMELY: RTT messages carry hash (flowId) and need to follow application message path
    ICMPMessage* icmpMsg = dynamic_cast<ICMPMessage*>(datagram->getEncapsulatedPacket());
    if (icmpMsg) {
        RTTMessage* rttMsg = dynamic_cast<RTTMessage*>(icmpMsg->getEncapsulatedPacket());
        if (rttMsg) {
            return rttMsg->getFlowId();
        }
    }
...
//        }
        uint32 hash = datagram->getSrcAddress().getInt() + datagram->getDestAddress().getInt() + udpPkt->getSrcPort() + udpPkt->getDestPort();
        return hash;
    }
