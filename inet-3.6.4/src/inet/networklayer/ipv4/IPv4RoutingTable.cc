...
void IPv4RoutingTable::findBestMatchingEqualCostRoutes(const IPv4Address& dest, vector<IPv4Route*>& ecRoutes) const
{
    Enter_Method_Silent();
    ...
    if (it != routingCache.end()) {
        ...
        for (auto e : routes) {
            if (e->isValid()) {
                if (IPv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask())) {    // match
                    if (ecRoutes.empty()) { // First matching route
                        matchingPrefixLength = e->getPrefixLength();
                    } else if (matchingPrefixLength != e->getPrefixLength()) {
                        break;
                    }
                    ecRoutes.push_back(const_cast<IPv4Route *>(e));
                }
            }
        }
    }
