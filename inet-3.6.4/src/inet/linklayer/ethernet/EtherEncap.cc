...
void EtherEncap::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        ecfSeqNum = 0;
        WATCH(seqNum);
        totalFromHigherLayer = totalFromMAC = totalPauseSent = totalEcfSent = totalEcfNackSent = 0;
        useSNAP = par("useSNAP").boolValue();
        WATCH(totalFromHigherLayer);
        WATCH(totalFromMAC);
        WATCH(totalPauseSent);
        WATCH(totalEcfSent);
        WATCH(totalEcfNackSent);
    }
...
void EtherEncap::handleMessage(cMessage *msg)
{
    if (msg->arrivedOn("lowerLayerIn")) {
        ...
        switch (msg->getKind()) {
            ...
            case IEEE802CTRL_SEND_ECF:
                // higher layer wants MAC to send Escape Control Frame
                handleSendECF(msg);
                break;

            case IEEE802CTRL_SEND_ECF_NACK:
                // higher layer wants MAC to send Escape Control Frame NACK
                handleSendECFNack(msg);
                break;
...
void EtherEncap::handleSendECF(cMessage* msg)
{
    Ieee802Ctrl* etherctrl = dynamic_cast<Ieee802Ctrl*>(msg->removeControlInfo());
    if (!etherctrl)
        throw cRuntimeError("ECF command `%s' from higher layer received without Ieee802Ctrl", msg->getName());
    uint32 flowId = etherctrl->getFlowId();
    delete etherctrl;

    EV_DETAIL << "Creating and sending ECF frame, with flowId = " << flowId << "\n";

    char framename[50];
    sprintf(framename, "EscapeControlFrame-%d-%d", getId(), ecfSeqNum++);
    EscapeControlFrame* frame = new EscapeControlFrame(framename);
    frame->setDest(MACAddress::MULTICAST_PAUSE_ADDRESS);
    if (frame->getByteLength() < MIN_ETHERNET_FRAME_BYTES)
        frame->setByteLength(MIN_ETHERNET_FRAME_BYTES);
    frame->setFlowId(flowId);
...
void EtherEncap::handleSendECFNack(cMessage* msg)
{
    Ieee802Ctrl* etherctrl = dynamic_cast<Ieee802Ctrl*>(msg->removeControlInfo());
    if (!etherctrl)
        throw cRuntimeError("ECF NACK command `%s' from higher layer received without Ieee802Ctrl", msg->getName());
    uint32 flowId = etherctrl->getFlowId();
    delete etherctrl;

    EV_DETAIL << "Creating and sending ECF NACK frame, with flowId = " << flowId << "\n";

//    char framename[50];
//    sprintf(framename, "EscapeControlFrameNACK-%d-%d", getId(), ecfSeqNum++);
//    EscapeControlFrameNACK* frame = new EscapeControlFrameNACK(framename);
//    frame->setDest(MACAddress::MULTICAST_PAUSE_ADDRESS);
//    if (frame->getByteLength() < MIN_ETHERNET_FRAME_BYTES)
//        frame->setByteLength(MIN_ETHERNET_FRAME_BYTES);
//    frame->setFlowId(flowId);
