...
void EtherMACFullDuplex::processMsgFromNetwork(cPacket *pk)
{
    ...
    else if (dynamic_cast<EscapeControlFrame*>(frame) != nullptr) {
        receiveEscapeFrame((EscapeControlFrame*)frame);
    }
    else if (dynamic_cast<EscapeControlFrameNACK*>(frame) != nullptr) {
            receiveEscapeFrameNACK((EscapeControlFrameNACK*)frame);
    }
    else {
...
void EtherMACFullDuplex::handleEndTxPeriod()
{
    ...
    onCurTxFrameComplete(curTxFrame);

    delete curTxFrame;
    curTxFrame = nullptr;
    lastTxFinishTime = simTime();

    if (pausePending && (pauseUnitsRequested > 0)) {
        // if we received a PAUSE frame recently, go into PAUSE state
        EV_DETAIL << "Going to PAUSE mode for " << pauseUnitsRequested << " time units\n";
...
void EtherMACFullDuplex::finish()
{
    ...
    recordScalar("numFramesSent", numFramesSent);
    recordScalar("numBytesSent", numBytesSent);
    recordScalar("numPauseFramesRcvd", numPauseFramesRcvd);
    recordScalar("numPauseFramesSent", numPauseFramesSent);
}
...
void EtherMACFullDuplex::handleEndPausePeriod()
{
    ...
    if (0 < currPauseUnits) { // Extend (x-off)
        scheduleEndPausePeriod(currPauseUnits);
    } else {
...
void EtherMACFullDuplex::processPauseCommand(int pauseUnits)
{
    ...
    if (transmitState == TX_IDLE_STATE) {
        EV_DETAIL << "PAUSE frame received, pausing for " << pauseUnits << " time units\n";
        //if (pauseUnits > 0)
            scheduleEndPausePeriod(pauseUnits);
    }
    else if (transmitState == PAUSE_STATE) {
        EV_DETAIL << "PAUSE frame received, pausing for " << pauseUnits
                  << " more time units from now\n";
        cancelEvent(endPauseMsg);

        /*
         * Handle UNPAUSE:
         * UNPAUSE if pauseUnits == 0; Extend PAUSE if pauseUnits > 0
         */
        scheduleEndPausePeriod(pauseUnits);
    }
    else {
...
void EtherMACFullDuplex::scheduleEndPausePeriod(int pauseUnits)
{
    ...
    EV_INFO << "End pause period scheduled to " << pauseUnits << "\n";
    currPauseUnits = pauseUnits;
}
