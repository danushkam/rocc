...
class INET_API EtherMACFullDuplex : public EtherMACBase
{
  ...
  protected:
    ...
    // Escape helpers (impl in the extended class)
    virtual void receiveEscapeFrame(EscapeControlFrame* frame) { delete frame; }
    virtual void receiveEscapeFrameNACK(EscapeControlFrameNACK* frame) { delete frame; }

    // statistics
    simtime_t totalSuccessfulRxTime;    // total duration of successful transmissions on channel

    bool pausePending = false;
    int currPauseUnits = 0;

    virtual void onCurTxFrameComplete(EtherFrame* curTxFrame) {}
