...
//    ss << flowId << " " << appPkt->getSequenceNumber() << " " << expectedSeq << endl;
    ...
    if (sendAck) {
        ...
        ackMsg->setNHop(appPkt->getNHop());
        for (int i = 0; i < appPkt->getNHop(); i++) {
            NodeINT& nodeInt = appPkt->getL(i);
            AckNodeINT& ackNodeInt = ackMsg->getL(i);
            ackNodeInt.B = nodeInt.B;
            ackNodeInt.qLen = nodeInt.qLen;
            ackNodeInt.ts = nodeInt.ts;
            ackNodeInt.txBytes = nodeInt.txBytes;
        }
    }
