...
static void
app_init_rocc(uint32_t port, struct switch_state *switch_state,
		bool enable, bool enable_tp_in, bool enable_tp_out)
{
	char fname[32];
	
	switch_state->enabled = enable;
	switch_state->r_curr = 0;
	switch_state->q_curr = 0;
	switch_state->q_old = 0;
	switch_state->tp_in.enabled = enable_tp_in;
	switch_state->tp_in.intvl_num_pkts = 0;
        switch_state->tp_in.intvl_num_bits = 0;
        switch_state->tp_in.intvl_start_time = 0;
	switch_state->tp_out.enabled = enable_tp_out;
       	switch_state->tp_out.intvl_num_pkts = 0;
        switch_state->tp_out.intvl_num_bits = 0;
        switch_state->tp_out.intvl_start_time = 0;
	sprintf(fname, "switch-stat-port-%u.txt", port);
	switch_state->stat_vector = fopen(fname, "w");
	if (switch_state->stat_vector == NULL)
		rte_panic("Cannot create stat vector file: %s\n", fname);
	switch_state->start_time = rte_rdtsc();
}
...
{
	uint32_t i;
	bool enable, enable_tp_in, enable_tp_out;
	...
	for (i = 0; i < APP_MAX_PORTS; i++) {
		...
		enable_tp_in = i != 0;
		enable_tp_out = false;
		app_init_rocc(i, &app.switch_state[i], enable, enable_tp_in, enable_tp_out);
	}
}
